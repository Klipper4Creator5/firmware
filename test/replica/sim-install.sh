...
# See test/replica/printer/case-install.sh and test/replica/printer-exec.sh.
set -uo pipefail
...
USB_STICK=1 BASE_PKG="$BASE" exec "$ROOT/test/replica/printer-exec.sh" \
    "$ROOT/test/replica/printer/case-install.sh" \
