...
#              `PRINTER_IMAGE=foo ./test/integration/sim-install.sh ...`, so the override
...
#              test/integration/build-printer-image.sh); PREBUILT=1 then. Otherwise the
#              local sim image is built from test/integration/printer/Dockerfile, which
