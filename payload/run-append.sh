...
if [ -n "$MODTAR" ]; then
    ...
    if [ "${FREE_KB:-0}" -lt "$NEED_KB" ]; then
        ...
        # all live in it.
        #
        # Two separate things used to destroy them on every update, which is
        # why fixing only one would not have helped: the rm -rf on the next
        # line takes the whole directory, and the tarball ships a seeded
        # config/settings.json of its own that would land on top anyway --
        # complete with "wizard_completed": false, so the printer came back
        # asking to be set up from scratch.
        #
