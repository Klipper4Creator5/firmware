...
# else. Pointed at the PAYLOAD, not a build tree, so nothing staged by an
# unknown path ships ungated. Nothing is exempt: qemu-mipsel-static does not
# enforce the check a real kernel's binfmt loader does, which is how a
# legacy-NaN binary once shipped with every gate green.
mips_abi_gate() {
    local n=0 f hdr counts total good
    while IFS= read -r f; do
        # readelf is the ELF test: a magic-byte compare made bash warn about
        # ignored null bytes on every data file that starts with a NUL.
        hdr=$(readelf -h "$f" 2>/dev/null) || continue
        # One header per MEMBER: readelf -h on a static archive prints one per
        # object, so every header must conform, and hex and words are matched on
        # the same awk line so one member's flags are not paired with another's
        # ISA. A mask, not the exact 0x70001405/07 pair: the low bits are
        # NOREORDER/PIC/CPIC (libarchive's xxhash.o reads 0x70001406, same ABI).
        counts=$(awk '
            /Flags:/ {
                total++
                fl = $2; sub(/,$/, "", fl)
        ...
        # total==0 is a failure: readelf saw the file and found nothing to check.
