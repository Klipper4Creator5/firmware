...
# choose and we do not. This is the seam -- one symlink per file, so $MODDIR
# stays the only place anything is installed and `opkg upgrade` is enough to
# change what the printer runs.
#
# ORDERING. Stock run.sh distributes the software component before
# run-append.sh extracts the payload, so the component cannot ship these
# symlinks itself: on a first install they would dangle, and on an upgrade
# they would resolve to the payload being replaced. This runs after
# extraction, and again from anvil-core's postinst. The component no longer
# carries firmwareExe or start.sh at all -- a wrapper without a payload behind
# it is worse than no wrapper, since app_startup.sh restores the stock binary
# while the wrapper would just sit there and never start a UI.
set -e

MODDIR=${MODDIR:-/usr/data/anvil}

# Only on a machine that has the paths below. The build installs this package
# with opkg too, and opkg runs maintainer scripts with IPKG_INSTROOT empty, so
# without this the postinst would act on whatever ran the build.
...
# two destinations under /usr/prog are the ones stock run.sh copies to, so
# this stays true as long as that does -- its two cp lines are no-ops now that
# the component ships neither file.
link_one() {
    src="$MODDIR/$1"
    dst="$2"
...
# including -- the path as opened, NOT the resolved target. So a symlinked
# printer.base.cfg still finds the stock printer.filament.cfg beside it, which
# is what lets those stay stock and unpackaged.
...
case "$MACHINE" in
    ...
    *)
        ...
        # guess. A missing include is a better failure than the wrong chamber
        # geometry.
