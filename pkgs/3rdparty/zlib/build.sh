...
# it used to be cross-built twice, once inside each.
#
# NOT AUTOTOOLS: zlib's configure is hand-written and has never accepted
# --host, which is the knob pkg_build would otherwise add. CHOST is what it
# does read, so PKG_CONFIGURE_AUTO=0 turns the flag off and the recipe
# exports CHOST instead -- a knob doing its job, not an exception to the
# rule. It used to BE one, allowed by name in qa/static/test_ipk.py; that
# rule has no exceptions now.
#
