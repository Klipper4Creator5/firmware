...
Everything in `tools/replica/printer/` exists to answer one question honestly: **would
...
`tools/replica/printer/binfmt.sh` registers a handler with that byte masked out.
...
pretending to be 1.0.2d is reassurance, not evidence — `tools/replica/printer/seed-prog.sh`
