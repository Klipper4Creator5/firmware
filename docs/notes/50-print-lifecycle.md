...
`[pause_resume]` and `[virtual_sdcard]`. `pkg/anvil-core/payload/config/ff-print-macros.cfg` reproduces the
sequences for Mainsail-started prints.

**How they are entered now.** `START_PRINT` is no longer something the slicer has to
call. `[ff_print]` (`pkg/klipper/prog/klippy/extras/ff_print.py`) wraps `SDCARD_PRINT_FILE` and
