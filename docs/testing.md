...
| `test-moonraker` | Loads moonraker's `authorization` component on the printer's own Python with the exact library path `S60web` sets, then takes libsodium away again and requires it to fail — the negative control for the bug where moonraker silently never started | yes |
