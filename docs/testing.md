...
| `test-moonraker` | Installs the payload the way an update does and then **runs** the shipped tools. `anvil-env.sh` must produce a working interpreter; moonraker's `authorization` component must import on it, and must fail again once libsodium is taken off the path — the negative control for the bug where moonraker silently never started. Then `init.d/S62moonraker` must actually start moonraker from `/usr/data/anvil/moonraker/moonraker.py` (checked in `/proc/<pid>/cmdline`, along with `TMPDIR` off the ramdisk), answer on :7125, report itself in `status`, clear its pidfile on `stop`, come back on a **new** pid after `restart` — and stopping it must leave `S60nginx` running, which is what splitting `S60web` in two was for | yes |
| `test-mcu` | Runs `ff_mcu_bringup.py` on the printer's **own** Python 3.8.2 in the exact environment `start.sh` sets — the only gate that executes our Python on the real interpreter, and the one that pins the `LD_LIBRARY_PATH` regression that shipped broken once | yes |
